...
def build_database(raw_dir: Path = RAW_DATA_DIR, db_path: Path = DATABASE_PATH,
    ...
    for filename, table in TABLE_MAP.items():
        ...
        if not fpath.exists():
            ...
            continue
        log.info(f"Loading {filename} -> table `{table}`")
        try:
            df = pd.read_csv(fpath)
        except UnicodeDecodeError:
            # Some Home Credit files (notably HomeCredit_columns_description.csv)
            # ship as Latin-1/cp1252, not UTF-8. Fall back gracefully.
            log.warning(f"{filename} is not UTF-8, retrying with latin1 encoding")
